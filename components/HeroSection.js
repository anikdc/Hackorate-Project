...
// 1️⃣ Your custom rotating word hook
...
// 2️⃣ Child component that uses the hook
function GameItem({ game, index, pay }) {
  const currentWord = useRotatingWords(game.rotating, 2000);
  ...
  return (
    ...
    >
      ...
      <div className="text-white text-center md:text-left space-y-4">
        ...
        <p className="text-md md:text-lg text-gray-300 max-w-md">
          {game.description}
        </p>
        <button
...
export default function HeroSection() {
  ...
  // Payment function remains the same
  const pay = async (gameTitle, amount) => {
    ...
    if (typeof window !== "undefined" && window.Razorpay) {
      ...
      if (!userKeyId) {
        toast.error(
          "Payment failed: You cannot make payment to this user because they are not in our database."
        );
        return;
      }
  ...
  return (
    <section
      id="hero"
      className="relative w-full min-h-screen overflow-hidden bg-black py-16"
    >
      {/* Background */}
      <div className="absolute inset-0 z-0">
        <Image
          src="/images/bg-game.jpg"
          alt="Background Game"
          layout="fill"
          objectFit="cover"
          quality={100}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-12 grid grid-cols-1 md:grid-cols-2 gap-12">
        {games.map((game, index) => (
          <GameItem key={index} game={game} index={index} pay={pay} />
        ))}
      </div>
    </section>
  );
